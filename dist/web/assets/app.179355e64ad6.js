...
function evaluate(expression, ctx) {
  ...
  const allowed=new Set(['state','variables','parameters','portfolio','changed','previous','target_deviation','weight_deviation','abs','min','max','sum','count','all','any','round','clamp','true','false','null',...Object.keys(ctx.market)]);
  ...
  return Function(...names,'state','variables','parameters','portfolio','changed','previous','target_deviation','weight_deviation',...Object.keys(helpers),'"use strict";return ('+code+')')(...values,ctx.state,ctx.variables,ctx.parameters,ctx.portfolio,ctx.changed,ctx.previous,ctx.targetDeviation,ctx.weightDeviation,...Object.values(helpers));
}
...
function applyImmediateUiState(state){
  ...
  delete document.documentElement.dataset.savedView;
...
if(savedUiState.indicatorTickerLabels&&typeof savedUiState.indicatorTickerLabels==='object')indicatorTickerLabels={...savedUiState.indicatorTickerLabels};
...
async function fetchProxyTickerData(data,tickers,historyStart=null,includeLabels=false){
  ...
  for(const ticker of [...new Set(tickers.filter(isDataProxyTicker))]){
    ...
    if(!includeLabels&&!requiredStart&&rows.length&&now-(proxyTickerFreshness.get(ticker)||0)<PROXY_REFRESH_INTERVAL_MS)continue;
  ...
  await Promise.all(batches.map(async([startDate,requested])=>{
    const requestKey=`${startDate}:${requested.slice().sort().join(',')}:${includeLabels?'labels':'prices'}`;
    if(proxyRequests.has(requestKey))return proxyRequests.get(requestKey);
    const request=(async()=>{
      const endpoint=new URL('/prices',proxyUrl);endpoint.searchParams.set('tickers',requested.join(','));endpoint.searchParams.set('start',startDate);if(includeLabels)endpoint.searchParams.set('include_labels','1');
      try{
        ...
        for(const [ticker,label] of Object.entries(payload.labels||{}))if(typeof label==='string'&&label.trim())indicatorTickerLabels[ticker]=label.trim();
...
async function addIndicatorComparisonTicker(value){
  ...
  await fetchProxyTickerData(data,[ticker],start,isIndicatorProductCode(ticker));
...
async function removeIndicatorComparisonTicker(ticker){
  ...
  delete indicatorTickerLabels[ticker];
