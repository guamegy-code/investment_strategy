...
function installFinalTargetRebalance(StrategyClass){const baseStep=StrategyClass.prototype.step;if(baseStep.__finalTargetRebalanceInstalled)return StrategyClass;function step(date,market,portfolio){const signal=baseStep.call(this,date,market,portfolio),match=String(signal.reason||'').match(/^DECLARATIVE_RULE_(\d+)(?:\s*\|\s*(.*))?$/);if(!match)return signal;const rule=this.def.rebalance?.[Number(match[1])-1];if(!rule||evaluate(rule.when,this.context(market,portfolio,signal.target)))return signal;const remainingReason=match[2]||null;return{...signal,rebalance:Boolean(remainingReason),reason:remainingReason};}step.__finalTargetRebalanceInstalled=true;StrategyClass.prototype.step=step;return StrategyClass;}
...
async function downloadMissingData(def){const data=await loadData(),{tickers}=validateStrategy(def),needsTdf=tickers.some(ticker=>ticker==='TDF2050_PROXY'||krwAdjustedBaseTicker(ticker)==='TDF2050_PROXY'),tdfComponents=needsTdf?Object.keys(TDF2050_PROXY_COMPONENT_WEIGHTS):[],refreshTickers=[...new Set([...tickers,...tdfComponents])];await loadCachedTickerData(data,refreshTickers);await fetchProxyTickerData(data,refreshTickers);await ensureTdf2050Proxy(data,tickers);await ensureKrwAdjustedAssets(data,tickers);const unavailable=tickers.filter(ticker=>!data[ticker]&&!isDataProxyTicker(ticker));if(unavailable.length)throw Error(`시장 데이터가 없습니다: ${unavailable.join(', ')}`);const incomplete=tickers.filter(ticker=>!data[ticker]);if(incomplete.length)throw Error(`시세를 찾을 수 없습니다: ${incomplete.join(', ')}`);refreshIndicatorTickerCatalog(data);return data;}
...
function backgroundRun(def,data,all){
  if(strategyDependencies(def,all).calculation.valuation)return Promise.resolve(runWithData(def,data,all));
