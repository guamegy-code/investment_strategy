...
const BROWSER_ENGINE_VERSION = '2026-08-24.1';
...
function runWithData(def,data,availableDefinitions=definitions,{removeFx=true}={}){const validation=validateStrategy(def,availableDefinitions),all=Object.fromEntries(availableDefinitions.map(item=>[item.strategy.id,item]));all[def.strategy.id]=def;const holdings=strategyHoldingTickers(def,availableDefinitions),rows=recordsFor(validation.tickers,data).filter(row=>validation.tickers.every(ticker=>[...(validation.fields[ticker]||[])].every(field=>Number.isFinite(Number(row.market[ticker][field])))));if(!rows.length)throw Error('지표 초기 계산 구간 이후에 공통 거래일이 남아 있지 않습니다.');const runtime=resolve(def,all,removeFx),portfolio=new Portfolio();for(const row of rows){portfolio.update(portfolioPrices(row.market,'open',removeFx,holdings),row.date);const signal=runtime.step(row.date,row.market,portfolio);if(!portfolio.history.length)portfolio.start(signal.target,signal.days,row.date,'INITIAL');else if(signal.rebalance)portfolio.start(signal.target,signal.days,row.date,'RULE');portfolio.record(row.date,portfolioPrices(row.market,'close',removeFx,holdings),signal.state);}const events=new Map(portfolio.rebalances.map(event=>[event.ExecutionDate||event.Date,event]));return portfolio.history.map(row=>{const event=events.get(row.date);return event?{...row,target:event.Target,preWeights:event.PreWeights||null,executionDays:event.ExecutionDays}:row;});}
...
function backgroundRun(def,data,all){
  ...
  const runtime=[
    ...
    definitionById,tickerFxRate,strategyDependencies,expressionStrings,requiredMarketFields,validateStrategy,pct,normalizeExpr,evaluate,period,recordsFor,marketRow,stateName,portfolioPrices,strategyHoldingTickers,resolve,Portfolio,Declarative,runWithData
