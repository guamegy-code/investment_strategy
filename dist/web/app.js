...
const graphConfig={responsive:true,displaylogo:false,scrollZoom:false,doubleClick:'reset',showAxisDragHandles:false,showAxisRangeEntryBoxes:false,modeBarButtonsToRemove:['lasso2d','select2d','zoom2d','zoomIn2d','zoomOut2d','autoScale2d','pan2d']};
...
const parityStyle=document.createElement('style');parityStyle.textContent='.offline-date-range{width:300px;gap:8px}.offline-date-range input{min-width:0;padding:6px 8px;font-size:13px}.research-kpi-card{position:relative;overflow:hidden}.research-kpi-icon{position:absolute;top:16px;right:16px;width:34px;height:34px;display:grid;place-items:center;border-radius:10px;font-size:18px;font-weight:700}.research-kpi-positive{color:#2fb344;background:rgba(47,179,68,.12)}.research-kpi-negative{color:#d63939;background:rgba(214,57,57,.12)}.research-kpi-primary{color:#206bc4;background:rgba(32,107,196,.12)}.research-kpi-cyan{color:#0ca8c0;background:rgba(12,168,192,.12)}';document.head.append(parityStyle);
...
runDashboard=async function(){try{$('status').textContent='저장된 전략 결과를 불러오고 있습니다…';const selected=selectedDefinitions();if(!selected.length)throw Error('전략을 하나 이상 선택하세요.');const cached=await loadPrecomputedResults(),settled=await Promise.allSettled(selected.map(async def=>{const saved=cached[def.strategy.id]||[];return [def,saved.length?saved:await run(def)];})),completed=settled.filter(result=>result.status==='fulfilled').map(result=>result.value),failed=settled.filter(result=>result.status==='rejected');if(!completed.length)throw failed[0].reason;dashboardResults=new Map(completed.map(entry=>[entry[0].strategy.id,entry]));renderAnalysis();$('status').textContent=failed.length?`${completed.length}개 전략 로드 · ${failed.length}개 실패: ${failed.map(item=>item.reason.message).join(' | ')}`:'';}catch(error){$('status').textContent=`오류: ${error.message}`;console.error(error);}};
...
setupDashboard=async function(){
  ...
  for(const input of $('strategy-list').querySelectorAll('input[type="checkbox"]')){
    ...
    input.onchange=()=>{saveUiState();runDashboard();};
  }
...
const productDisplayNames={'379810.KS':'KODEX 미국나스닥100','426030.KS':'TIME 미국나스닥100액티브','0015B0.KS':'KoAct 미국나스닥성장액티브','434060.KS':'KODEX TDF2050액티브'};
...
async function updateDetailFxView(){
  ...
  try{
    const data=await loadData(),history=runWithData(entry[0],data,definitions,{removeFx:false});
    detailFxHistories.set(id,history);
    renderDetail();
  }catch(error){
    toggle.checked=true;
    $('status').textContent=`환율 효과를 포함한 성과를 계산하지 못했습니다: ${error.message}`;
...
setupDashboard=async function(){
  ...
  $('start-date').onchange=refreshAnalysisRange;
  $('end-date').onchange=refreshAnalysisRange;
};
...
function isDataProxyTicker(ticker){return DATA_PROXY_TICKER_PATTERN.test(String(ticker||'').toUpperCase());}
function strategyDependencies(def, all=definitions, seen=new Set()){
  ...
  const base=def.source?strategyDependencies(source,all,nextSeen):{calculation:source,tickers:[...(source.assets?.required||[])]};
  const tickers=[...new Set([...base.tickers,...Object.values(def.products||{}).flatMap(value=>Object.keys(value||{}))])];
  for(const ticker of [...tickers]){const fx=tickerFxRate(ticker);if(fx)tickers.push(fx);}
...
async function loadCachedTickerData(data,tickers){for(const ticker of tickers)if(!data[ticker]){const cached=await cachedPrices(ticker);if(cached)data[ticker]=cached.rows||cached;}}
...
async function fetchProxyTickerData(data,tickers){const groups=new Map();for(const ticker of [...new Set(tickers.filter(isDataProxyTicker))]){const start=latestCachedDate(data[ticker]||[])||'2010-01-01';(groups.get(start)||groups.set(start,[]).get(start)).push(ticker);}if(!groups.size)return;if(!proxyUrl){const unavailable=[...groups.values()].flat().filter(ticker=>!data[ticker]);if(unavailable.length)throw Error(`시장 데이터가 없습니다: ${unavailable.join(', ')}`);return;}for(const [startDate,group] of groups)for(let offset=0;offset<group.length;offset+=20){const requested=group.slice(offset,offset+20),endpoint=new URL('/prices',proxyUrl);endpoint.searchParams.set('tickers',requested.join(','));endpoint.searchParams.set('start',startDate);const response=await fetch(endpoint),payload=await response.json();if(!response.ok)throw Error(payload.error||'시장 데이터 다운로드에 실패했습니다.');for(const [ticker,rows] of Object.entries(payload.data||{})){const incoming=rows.map(row=>({Date:row.date,Open:row.open,High:row.high,Low:row.low,Close:row.close,Volume:row.volume})),merged=mergeTickerRows(data[ticker],incoming);data[ticker]=merged;await saveCachedPrices(ticker,merged);}}}
...
async function ensureTdf2050Proxy(data,tickers){if(!tickers.includes('TDF2050_PROXY'))return;const components=Object.keys(TDF2050_PROXY_COMPONENT_WEIGHTS),unavailable=components.filter(ticker=>!data[ticker]);if(unavailable.length)throw Error(`TDF2050_PROXY 구성 데이터를 찾을 수 없습니다: ${unavailable.join(', ')}`);data.TDF2050_PROXY=addStrategyIndicators(buildTdf2050Proxy(data));await saveCachedPrices('TDF2050_PROXY',data.TDF2050_PROXY);}
async function downloadMissingData(def){const data=await loadData(),{tickers}=validateStrategy(def),tdfComponents=tickers.includes('TDF2050_PROXY')?Object.keys(TDF2050_PROXY_COMPONENT_WEIGHTS):[],refreshTickers=[...new Set([...tickers,...tdfComponents])];await loadCachedTickerData(data,refreshTickers);await fetchProxyTickerData(data,refreshTickers);await ensureTdf2050Proxy(data,tickers);const unavailable=tickers.filter(ticker=>!data[ticker]&&!isDataProxyTicker(ticker));if(unavailable.length)throw Error(`시장 데이터가 없습니다: ${unavailable.join(', ')}`);const incomplete=tickers.filter(ticker=>!data[ticker]);if(incomplete.length)throw Error(`시세를 찾을 수 없습니다: ${incomplete.join(', ')}`);refreshIndicatorTickerCatalog(data);return data;}
function portfolioPrices(market,field='close',removeFx=true){const prices={};for(const [ticker,row] of Object.entries(market)){const fxTicker=tickerFxRate(ticker),fx=removeFx&&fxTicker?Number(market[fxTicker]?.[field]):1;prices[ticker]=Number(row[field])/(Number.isFinite(fx)&&fx>0?fx:1);}return prices;}
...
function runWithData(def,data,availableDefinitions=definitions,{removeFx=true}={}){const validation=validateStrategy(def,availableDefinitions),all=Object.fromEntries(availableDefinitions.map(item=>[item.strategy.id,item]));all[def.strategy.id]=def;const rows=recordsFor(validation.tickers,data).filter(row=>validation.tickers.every(ticker=>[...(validation.fields[ticker]||[])].every(field=>Number.isFinite(Number(row.market[ticker][field])))));if(!rows.length)throw Error('지표 초기 계산 구간 이후에 공통 거래일이 남아 있지 않습니다.');const runtime=resolve(def,all,removeFx),portfolio=new Portfolio();for(const row of rows){portfolio.update(portfolioPrices(row.market,'open',removeFx),row.date);const signal=runtime.step(row.date,row.market,portfolio);if(!portfolio.history.length)portfolio.start(signal.target,signal.days,row.date,'INITIAL');else if(signal.rebalance)portfolio.start(signal.target,signal.days,row.date,'RULE');portfolio.record(row.date,portfolioPrices(row.market,'close',removeFx),signal.state);}const events=new Map(portfolio.rebalances.map(event=>[event.ExecutionDate||event.Date,event]));return portfolio.history.map(row=>{const event=events.get(row.date);return event?{...row,target:event.Target,preWeights:event.PreWeights||null,executionDays:event.ExecutionDays}:row;});}
async function resultKey(def,data){const market=strategyDependencies(def).tickers.map(ticker=>[ticker,(data[ticker]||[]).map(row=>[row.Date,row.Open,row.High,row.Low,row.Close,row.Volume])]);const bytes=new TextEncoder().encode(JSON.stringify([BROWSER_ENGINE_VERSION,bundle.market_data_version||'',def,market]));const digest=await crypto.subtle.digest('SHA-256',bytes);return [...new Uint8Array(digest)].map(value=>value.toString(16).padStart(2,'0')).join('');}
function backgroundRun(def,data,all){
  ...
  const runtime=[
    ...
    definitionById,tickerFxRate,strategyDependencies,expressionStrings,requiredMarketFields,validateStrategy,pct,normalizeExpr,evaluate,period,recordsFor,marketRow,stateName,portfolioPrices,resolve,Portfolio,Declarative,runWithData
...
async function run(def){const data=await downloadMissingData(def),key=await resultKey(def,data),cached=await cachedResult(key).catch(()=>null);if(cached?.history)return cached.history;let history;try{history=await backgroundRun(def,data,definitions);}catch(error){console.warn('Worker calculation unavailable; using the main thread.',error);history=runWithData(def,data);}saveCachedResult(key,{history,savedAt:new Date().toISOString()}).catch(()=>{});return history;}
...
setupDashboard=async function(){
  ...
  for(const input of $('strategy-list').querySelectorAll('input[type="checkbox"]')){
    ...
    input.closest('label')?.classList.toggle('research-product-strategy',Boolean(definition?.source));
  }
  $('import').onchange=async event=>{
    try{
      const def=migrateStrategyDefinition(parseYaml(await event.target.files[0].text()));
      if(!def?.strategy?.id)throw Error('strategy.id가 필요합니다.');
  ...
  if(fxToggle)fxToggle.onchange=updateDetailFxView;
};
localizeDashboard();
...
async function loadHostedStrategies(){
  ...
  const loaded=await Promise.all(entries.map(async entry=>{
    ...
    return parseYaml(await yamlResponse.text());
  }));
  definitions=loaded;
}
