...
class Backtest:
    ...
    def get_market(self, row):
        ...
        for ticker in self.tickers:
            market[ticker] = {
                ...
                "MA20": row.get(f"{ticker}_MA20", None),
                "MA55": row.get(f"{ticker}_MA55", None),
                "MA120": row.get(f"{ticker}_MA120", None),
                "MA200": row.get(f"{ticker}_MA200", None),
                "RSI14": row.get(f"{ticker}_RSI14", None),
            }
        return market
    ...
    def run(self):
        ...
        for date, row in self.data.iterrows():
            ...
            decision = self.strategy.evaluate(date, market, self.portfolio)
            ...
            if first_trade:
                self.portfolio.rebalance(prices, decision["target"], date)
                first_trade = False
