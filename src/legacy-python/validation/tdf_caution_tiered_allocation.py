"""Compare explicit BULL, CAUTION, and strong-CAUTION TDF tiers."""
...
def tiered_definition(bull_share, caution_share, strong_share=0.0):
    definition = caution_definition(0.40, 1.0)
    definition["state"]["safe_tdf_share"] = {
        "initial": f"{bull_share * 100}%",
        "rules": [
            {
                "when": (
                ...
                "set": f"{strong_share * 100}%",
                "confirm": 3,
            },
            {
                "when": "state.market_mode == 'CAUTION'",
                "set": f"{caution_share * 100}%",
            },
            {
                "when": "state.market_mode == 'BULL'",
                "set": f"{bull_share * 100}%",
            },
...
def main():
    ...
    definitions = {
        f"BULL {bull:.0%} / CAUTION {caution:.0%} / STRONG {strong:.0%}": (
            tiered_definition(bull, caution, strong)
        )
        for bull, caution, strong in tiers
    }
    ...
    summary.to_csv(
        RESULT_DIR / "tdf_caution_tiered_allocation.csv", index=False
    )
    rolling.to_csv(
        RESULT_DIR / "tdf_caution_tiered_allocation_rolling.csv", index=False
    )
