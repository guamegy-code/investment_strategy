"""Test a two-step recovery unwind from strategy 21's stage 2 allocation."""
...
BASELINE = Parameters(0.87, RECOVERY_2_DRAWDOWN, RECOVERY_2_DRAWDOWN)
...
def simulate(data: pd.DataFrame, parameters: Parameters):
    ...
    for index in range(len(data)):
        ...
        if partial:
            ...
            stage = 3
        elif stage == 2 and price >= peak * (1.0 - parameters.first_recovery_drawdown):
            if parameters.partial_weight <= TARGETS[1]:
                stage = 1
            else:
                partial = True
...
def main():
    ...
    rows = [{**item.__dict__, **simulate(full, item)} for item in candidates()]
