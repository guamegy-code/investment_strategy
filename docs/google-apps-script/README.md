...
DEFENSE·BEAR·RECOVERY 관련 상태/확인 변화와 목표 괴리 5%p 최초 돌파만 사전주의로 보냅니다.
같은 기준일의 주간·사전주의·실행 알림은 한 메시지로 합칩니다.

메시지에는 추세와 밸류에이션 상태, QQQ 1·5·20일 변화, 약세·회복 점수, 진행 중인 확인,
현재·목표·증감 비중이 포함됩니다. `setupSpreadsheet()`를 다시 실행하면 기존 자료를 유지한 채
`알림 이력`의 새 알림 유형·시장 요약 열을 준비합니다.
...
`source`/`products` 상품연결 전략은 원본의 상태와 시장 신호를 상속하고 실제 상품 비중으로
알림을 만듭니다. 원본과 연결 전략은 서로 다른 구독이므로, 같은 계좌에서는 둘 중 실제 운용할
전략 ID만 체크해 중복 알림을 피합니다.
