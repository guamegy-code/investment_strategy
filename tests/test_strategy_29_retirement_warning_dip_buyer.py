...
class Strategy29RetirementWarningDipBuyerTests(unittest.TestCase):
    def strategy(self):
        return DeclarativeStrategy.from_yaml(
            ROOT / "strategies" / "29_band_7030_tdf_valuation_warning_dip_buyer.yaml"
        )

    def test_warning_builds_twenty_percent_bil_buffer(self):
    ...
    def test_first_dip_restores_normal_seventy_thirty_allocation(self):
        strategy = self.strategy()
        portfolio = PortfolioStub()
        start = pd.Timestamp("2025-01-02")
        strategy.evaluate(start, observations(100.0, 70), portfolio)
        ...
        expected = {"QQQ": 0.70, "TDF2050_PROXY": 0.30, "BIL": 0.0}
        self.assertEqual(first["target"], expected)
        self.assertEqual(second["target"], expected)
        self.assertEqual(third["target"], expected)
    ...
    def test_risk_asset_target_never_exceeds_seventy_percent(self):
        ...
        for offset, observation in enumerate((
            observations(100.0, 70),
            observations(95.0, 45),
            observations(90.0, 45),
            observations(80.0, 45),
        )):
            signal = strategy.evaluate(
                start + pd.offsets.BDay(offset), observation, portfolio
            )
            self.assertLessEqual(signal["target"]["QQQ"], 0.70)
            self.assertAlmostEqual(sum(signal["target"].values()), 1.0)

    def test_dash_loader_exposes_strategy_29(self):
        identities = {
            strategy_identity(strategy) for strategy in load_active_strategies()
        }
        self.assertIn(
            "dsl:band-7030-tdf-valuation-warning-dip-buyer",
            identities,
        )
