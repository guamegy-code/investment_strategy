...
class NotificationWorkerContractTests(unittest.TestCase):
    def test_worker_exposes_authenticated_notification_evaluation(self):
        ...
        self.assertIn("mapped_products: Boolean(latestContext?.mapped_products)", source)
    ...
    def test_apps_script_uses_worker_for_evaluation_and_keeps_secrets_out_of_sheet(self):
        ...
        self.assertIn("function scheduledSummaryDue_(summary, date)", source)
        self.assertIn("function isKrxTradingDay_(dateOrKey)", source)
        self.assertIn("SKIPPED_KRX_CLOSED", source)
        ...
        self.assertIn("function formatStatesForSheet_(event)", source)
        self.assertIn("function formatMarketForSheet_(event)", source)
        self.assertEqual(source.count("function messageFor_(event)"), 1)
        self.assertNotIn("risk_off_score", source)
        self.assertNotIn("recovery_score", source)
        self.assertNotIn("market?.qqq", source)
        self.assertNotIn("trend_mode:", source)
        ...
        self.assertIn("parse_mode: 'HTML'", source)
        self.assertNotIn("truncateDisplay_", source)
        self.assertIn("'[[B]]📌 시장 상태[[/B]]'", source)
        self.assertIn("'[[B]]📊 신호[[/B]]'", source)
        self.assertIn("[[B]]📈 시장 지표 (${ticker})[[/B]]", source)
