...
function evaluate(expression,ctx){if(typeof expression!=='string')return expression;const tokens=expressionTokens(expression);let index=0,peek=()=>tokens[index],take=()=>tokens[index++],accept=type=>peek().type===type?(take(),true):false;const helpers={abs:Math.abs,min:Math.min,max:Math.max,sum:(...values)=>values.reduce((sum,value)=>sum+value,0),count:(...values)=>values.filter(Boolean).length,all:(...values)=>values.every(Boolean),any:(...values)=>values.some(Boolean),round:(value,digits=0)=>Number(value.toFixed(digits)),clamp:(value,low,high)=>Math.max(low,Math.min(high,value)),changed:ctx.changed,previous:ctx.previous,target_deviation:ctx.targetDeviation,weight_deviation:ctx.weightDeviation};function primary(){const token=take();if(token.type==='value')return token.value;if(token.type==='('){const value=parse(0);if(!accept(')'))throw Error('Expected )');return value;}if(token.type==='['){const values=[];if(!accept(']')){do{values.push(parse(0));}while(accept(','));if(!accept(']'))throw Error('Expected ]');}return values;}if(token.type==='name'){if(accept('(')){if((token.value==='changed'||token.value==='previous')&&peek().type==='name'){const field=take().value.split('.').at(-1);if(!accept(')'))throw Error('Expected )');return helpers[token.value](field);}const args=[];if(!accept(')')){do{args.push(parse(0));}while(accept(','));if(!accept(')'))throw Error('Expected )');}const helper=helpers[token.value];if(!helper)throw Error(`Unknown DSL function: ${token.value}`);return helper(...args);}return expressionValue(token.value,ctx);}if(token.type==='-'||token.type==='!')return token.type==='-'?-primary():!primary();throw Error(`Unexpected DSL token: ${token.type}`);}const precedence={'||':1,'&&':2,'==':3,'===':3,'!=':3,'!==':3,'>':3,'>=':3,'<':3,'<=':3,in:3,'+':4,'-':4,'*':5,'/':5};function parse(minimum){let left=primary();while(precedence[peek().type]>=minimum){const op=take().type,right=parse(precedence[op]+1);if(op==='||')left=left||right;else if(op==='&&')left=left&&right;else if(op==='in')left=Array.isArray(right)&&right.includes(left);else if(op==='=='||op==='===')left=left===right;else if(op==='!='||op==='!==')left=left!==right;else if(op==='>')left=left>right;else if(op==='>=')left=left>=right;else if(op==='<')left=left<right;else if(op==='<=')left=left<=right;else if(op==='+')left=left+right;else if(op==='-')left=left-right;else if(op==='*')left=left*right;else left=left/right;}return left;}const result=parse(0);if(peek().type!=='end')throw Error(`Unexpected DSL token: ${peek().type}`);return result;}
